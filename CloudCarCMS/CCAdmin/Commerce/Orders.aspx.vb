...
Namespace CCAdmin.Commerce
    ...
    Partial Public Class Orders
        ...
        Private Sub btnSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSearch.Click
            ...
            Dim SearchApproval As Boolean = Integer.TryParse(ddlApproved.SelectedValue, ApprovalState)
            Dim SearchShipped As Boolean = Integer.TryParse(ddlShipped.SelectedValue, ShippedState)


            Dim oList As List(Of EStore_SimpleOrderView) = OrderController.GetSimpleOrderViewsFunc(db).ToList

            If SearchOrderID And SearchApproval And SearchShipped Then
                oList = oList.Where(Function(p) p.ID.Value = OrderID And p.User.ToLower.Contains(UserSearch.ToLower) And p.ApprovalState.Value = ApprovalState And p.Shipped = ShippedState).ToList
